...
const BearCard = props => {
    ...
    const deleteBear = async () => {
        const result = await axios.delete(`http://localhost:${process.env.REACT_APP_PP}/api/bears/${props.id}`)
        dispatch({type : 'DELETE_BEAR', id: props.id})
      }
    
    const updateBear = async () => {
    const result = await axios.put(`http://localhost:${process.env.REACT_APP_PP}/api/bears/${props.id}`,form)
